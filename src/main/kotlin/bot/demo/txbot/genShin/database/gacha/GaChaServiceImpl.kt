 ...
 */
@Service
class GaChaServiceImpl : ServiceImpl<GaChaMapper?, GaChaEntity?>(), GaChaService {
    @Autowired
...
//    }
    ...
    override fun insertByUid(
        ...
        val queryWrapper = QueryWrapper<GaChaEntity>().eq("item_id", itemId)
        val existGachaInfo = gaChaMapper.selectOne(queryWrapper)
        if (existGachaInfo == null) gaChaMapper.insert(gachaInfo)
    }
}
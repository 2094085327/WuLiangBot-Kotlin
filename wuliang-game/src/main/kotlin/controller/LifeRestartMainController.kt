...
import config.TALENT_SELECT_Limit
import config.TALENT_SELECT_NOT_COMPLETE
...
import service.LifeRestartService
import utils.LifeRestartUtil
...
class LifeRestartMainController @Autowired constructor(
    private val lifeRestartService: LifeRestartService,
    ...
    fun lifeRestart(@RequestParam("game_userId") userId: String): Pair<MutableMap<String, Int>, Any?> {
        val userInfo = redisService.getValue("lifeRestart:userInfo:${userId}", UserInfoEntity::class.java)
